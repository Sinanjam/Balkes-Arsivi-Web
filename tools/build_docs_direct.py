...
BAD_RE = re.compile(r"(font-family:|\.slogan|\.nav1|https?://|www\.|web\.archive\.org|balkesarsivi\.com|/user/cimage|user/cimage|cimage/)", re.I)
...
def clean(s):
    ...
    s = s.replace("S ampiyon", "Şampiyon").replace("Ş ampiyon", "Şampiyon")
    s = s.replace("Oynadıgımız", "Oynadığımız")
    s = s.replace("Ilk Yarı", "İlk Yarı").replace("Ikinci Yarı", "İkinci Yarı")
    s = re.sub(r"[ \t]+", " ", s)
    s = re.sub(r"\n{3,}", "\n\n", s)
    return s.strip()
...
def paragraphs(x):
    ...
    for raw in re.split(r"\n\s*\n", content):
        ...
        if re.match(r"^\s*Tablo\s+\d+\s*$", p, re.I):
            continue
        if "|" in p and len(p) < 700:
            continue
        out.append(p)
    if not out:
        fallback = clean(x.get("summary", ""))
        if fallback:
            out.append(fallback)
    return out or ["Bu sayfa için okunabilir metin kısa olduğu için içerik arşiv kaydı olarak korunmuştur."]
...
def parse_tables(md):
    ...
    for title, body in chunks:
        ...
        if len(first) == 1 and len(rows) > 1:
            t = first[0]
        tables.append({"title": clean(t), "rows": rows})
    return tables

def normalize(raw_items):
    out = []
    seen = set()
    for idx, x in enumerate(raw_items, 1):
        title = clean(x.get("title", ""))
        summary = clean(x.get("summary", ""))
        content = clean(x.get("content", ""))
        if not title or BAD_RE.search(title) or BAD_RE.search(summary[:300]):
            continue
        ...
        # Uygulama verisindeki asıl photos dizisi.
        for p in x.get("photos") or []:
            ...
            if not asset or asset in used:
                continue
            used.add(asset)
            photos.append({"asset": asset, "caption": clean(p.get("caption", "")) or "Arşiv görseli"})

        # Kapak görseli photos içinde yoksa başa al.
        image_asset = str(x.get("imageAsset") or "").strip()
        if image_asset and image_asset not in used:
            photos.insert(0, {"asset": image_asset, "caption": clean(x.get("imageCaption", "")) or "Arşiv görseli"})
        ...
        tables = parse_tables(x.get("tables", ""))
        out.append({
            "id": sid,
            "season": clean(x.get("season", "")),
            "title": title,
            "summary": snippet(summary or content),
...
def write_site(items):
    if OUT.exists():
        shutil.rmtree(OUT)
    OUT.mkdir(parents=True, exist_ok=True)
    (OUT / ".nojekyll").write_text("", encoding="utf-8")

    payload = {"rawBase": RAW_BASE, "maxInlineTables": MAX_INLINE_TABLES, "items": items}
    (OUT / "data.js").write_text("window.BALKES_DATA=" + json.dumps(payload, ensure_ascii=False, separators=(",", ":")) + ";\n", encoding="utf-8")

    css = r"""
:root{--bg:#0d0a0b;--panel:#171214;--panel2:#21181b;--line:#3d262c;--text:#f7f1ef;--muted:#cdbfba;--red:#c8102e;--red2:#8f0b1b}
...
"""
    ...
    js = r"""
let DATA=window.BALKES_DATA||{items:[],rawBase:"",maxInlineTables:8};
let ITEMS=DATA.items||[], RAW=DATA.rawBase||"", MAXT=DATA.maxInlineTables||8;
...
function generatedBlock(){return `<div class="photo-block"><div class="placeholder">1966</div><div class="caption">Temsilidir</div></div>`}
...
function appArticleFlow(it){
  ...
  let photoCount=(it.photos||[]).length, photoLimit=Math.min(photoCount,Math.max(1,Math.min(8,Math.floor(ps.length/2)+1)));
  ...
  for(let i=0;i<ps.length;i++){
    ...
    if(i===0){ if(photoCount>0&&photoIndex<photoLimit){out+=photoBlock(it,photoIndex,photoCount);photoIndex++} else if(photoCount===0){out+=generatedBlock();generatedShown=true} }
...
function boot(){document.querySelector("#count").textContent=ITEMS.length+" içerik";search.addEventListener("input",renderList);renderList();if(location.hash.length>1){openItem(decodeURIComponent(location.hash.slice(1)))}}
boot();
"""
...
def main():
    if not ARCHIVE_JSON.exists():
        print("HATA: archive_items.json bulunamadı:", ARCHIVE_JSON)
        sys.exit(1)

    raw = json.loads(ARCHIVE_JSON.read_text(encoding="utf-8"))
    items = normalize(raw.get("items", []))
    if not items:
        print("HATA: hiç içerik üretilemedi")
        sys.exit(1)
    photos = sum(len(x["photos"]) for x in items)
    tables = sum(len(x["tables"]) for x in items)
    if photos == 0:
        print("HATA: fotoğraf referansı yok")
        sys.exit(1)

    write_site(items)
    print(f"Direct docs site üretildi: {len(items)} içerik, {photos} foto referansı, {tables} tablo")
